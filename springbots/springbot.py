...
class Springbot(object):
	...
	def boundingBox(self, nodeRadius=RADIUS):
		"""
	...
	def draw(self, screen, ticks=None, track_x=False, track_y=False, 
		...
		for x in xrange(0,width+100,siz_x):
			for y in xrange(0,height+100,siz_y):
				pygame.draw.rect(screen, backgroundcolor, (((x - cxp) % (width+siz_x) - siz_x, (y - cyp) % (height+siz_y) - siz_y), (siz_x-10,siz_y-10)))
		...
		for node in self.nodes:
			...
			pygame.draw.circle(screen, (10,255,255), (int(node.pos.x*zm - cx), int(node.pos.y*zm - cy)), int(RADIUS*zm), int(2*zm))
		...
		if showText:
			...
			speedtext = font.render("speed: %.2f m/s" % (sqrt(velx**2 + vely**2)), False, (255,255,255))

			screen.blit(nametext, (5,5))
			screen.blit(speedtext, (5,20))

			if ticks:
				tickstext = font.render("clock: %d ms" % (ticks), False, (255,255,255))
...
def store_xml(springbots, file=sys.stdout):
	"""
	Writes a set os springbots into a xml
	"""
	closefile = (type(file) is str)
	if closefile:
		file = open(file, 'w')
	...
	for springbot in springbots:
		...
		for spring in springbot.springs:
			...
			if fabs(spring.normal - sqrt(sum((spring.a.pos - spring.b.pos)**2))) > 1:
				file.write('normal="%.3f" ' % (spring.normal))
			file.write('/>\n')
...
def load_xml(file=sys.stdin, limit=None):
	...
	for e in doc.childNodes:
		if e.nodeType is xml.dom.minidom.Node.ELEMENT_NODE:
			springbotsNode = e
			break
	else:
		...
		if str(springbotNode.nodeName) == "springbot":
			...
			for e in springbotNode.childNodes:
				if str(e.nodeName) == "node":
					...
					# Cria spring
					offs = float(str(e.attributes["offset"].value)) if e.hasAttribute("offset") else pi
					ampl = float(str(e.attributes["amplitude"].value)) if e.hasAttribute("amplitude") else 0.0
					norm = float(str(e.attributes["normal"].value)) if e.hasAttribute("normal") else None
